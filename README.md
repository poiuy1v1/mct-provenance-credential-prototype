...
- provides a Windows/Ubuntu CI matrix and package-integrity validation.

The frozen SI value `26.2855` has no bundled executable provenance. Main and SI
were not edited by this software release; a later manuscript stage must align
the document with the executable value.

## Validation

Release acceptance requires Python 3.11+ and the exact CI requirements:
...
generated runs. The committed executed notebook is acceptable only when
generated through the `nbclient` backend. A standard-library execution route
may be used for offline diagnosis but is not release-acceptance evidence.
